...
| 环境 | 方式 | 结果 |
|---|---|---|
| SillyTavern 1.12.3（托管实例真机） | 无头 Chrome + CDP 注入源码运行 | 16 项检查 15 通过，唯一失败项是该实例自身的扩展自动更新连不上 GitHub（与本扩展无关） |
| SillyTavern 1.18.0（本机，临时数据目录） | 无头 Chrome + CDP 真实安装加载 | 14 项检查 13 通过，失败项同上 |

覆盖：扩展执行、面板挂载、API 自检、HTML→沙箱 iframe、iframe 内容与自动高度、
JSON Patch 覆盖、嵌套变量压平进原生变量表、数组追加、控制块隐藏、变量表注入、无未捕获异常。
